...
from os import pardir, makedirs, walk, environ
...
class Data(object):
    ...
    @staticmethod
    def get_tests(directory, deep_description = False):
        '''
    ...
    def availableTests(self):
        self._availableTests = get_tests(paths['dataDirectory'])
        return self._availableTests

    def tests_summary(self):
        return get_tests(self.paths['dataDirectory'], deep_description = True)
    ...
    def load(self, **kwargs):
        ...
        if 'tests' not in kwargs and 'devices' not in kwargs: 
            std_out('Need at least devices or tests to load', 'ERROR')

        load_ok = True
        if 'tests' in kwargs:
            for test in kwargs['tests']: 
                load_ok &= self.load_test(kwargs['tests'], options = options)
        ...
        if 'devices' in kwargs:
            ...
            load_ok &= self.load_devices(test_name, tdevices, options = options)

        return load_ok

    def load_test(self, test_name, options = dict()):
        
        _test = test.Test(test_name)
        _test.load(options)
        
        self.tests[test_name] = _test
    ...
    def load_devices(self, test_name, devices = list(), options = dict()):
        ...
        _test = test.Test(tname)
        for device in devices:
            _test.add_device(device)

        if not _test.make(): return
        _test.load(options)

        self.tests[tname] = _test
    ...
    def process(self):
        process_ok = True
        for test in self.tests: process_ok &= self.process_test(test)
    ...
    def upload_to_zenodo(self, upload_descriptor_name, sandbox = True, dry_run = True):
        ...
        if 'zenodo_token' not in environ:
            std_out('Need of zenodo_token in .env file', 'ERROR')
            return None
